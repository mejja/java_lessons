...
public class Main3 {
    public static void main(String[] args) {
        ...
        // Predicate to exclude people from a specific city
        Predicate<Person> isNotInCity = person -> person.getCity() != City.JOHANNESBURG;

        // Filter the list of people
        List<Person> over50sAndFemale = people.stream()
                                              .filter((isOver50).or(isFemale))
                                              .filter(isNotInCity)
                                              .collect(Collectors.toList());

        // Print the filtered list
        System.out.println("Female Or Over50 Peoples and Not in JOHANNESBURG:");
